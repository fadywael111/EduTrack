...
function copyPhone(phone, btn) {
    ...
    navigator.clipboard.writeText(display).then(() => {
        btn.innerHTML = '✓ ' + translations[currentLang].copy_success;
        btn.classList.add('copied');
        setTimeout(() => {
            btn.innerHTML = COPY_ICON + ' ' + translations[currentLang].copy;
...
function waButton(phone) {
    ...
    return `<div class="contact-actions">`
        + `<a class="wa-btn" href="https://wa.me/${phone}" target="_blank" rel="noopener">${WA_ICON} ${translations[currentLang].contact}</a>`
        + `<button class="copy-btn" onclick="copyPhone('${phone}', this)">${COPY_ICON} ${translations[currentLang].copy}</button>`
        + `</div>`;
}
...
function changeLevel(level) {
    ...
    // Translate level for storage/key purposes
    const translatedPrefix = currentLang === 'ar' ? 'المستوى' : 'Level';
    const translatedLevel = level.replace('Level', translatedPrefix);
    const textEl = document.getElementById('currentLevelText');
    if (textEl) textEl.textContent = translatedLevel;
    else document.getElementById('currentLevelBtn').textContent = translatedLevel;

    document.querySelectorAll('.level-item').forEach(btn => {
        const btnText = btn.textContent.trim();
        btn.classList.toggle('active', btnText === translatedLevel || btnText === level);
    });
    ...
    const currUnits = document.getElementById('curriculumUnits');
    ...
    document.querySelectorAll('.lesson-card').forEach(card => {
        ...
        // Reset info text
        if (info) {
            info.textContent = translations[currentLang].no_files;
        ...
        if (!card.querySelector('.upload-btn') && isLevelAdmin()) {
            ...
            btn.innerHTML = `
                ...
                </svg> ${translations[currentLang].upload}
            `;
    ...
    if (currUnits) currUnits.classList.remove('hidden');
...
function initClassFilter() {
    ...
    if (sel) {
        sel.innerHTML = '<option value="all">كل الفصول</option>';
        classes.forEach(c => {
            const opt = document.createElement('option');
            opt.value = c;
            opt.textContent = 'فصل ' + c;
            sel.appendChild(opt);
        });
    }
}
            ...
            <div class="card-header">
                <div class="card-title-group">
                    ...
                    <div class="card-subtitle">
                        <span class="class-label">فصل ${s.cls}</span>
                        <span class="sep">•</span>
                        <span class="guardian-name">${s.guardian || 'بدون ولي أمر'}</span>
                    </div>
                </div>
...
function render() {
    ...
    document.getElementById('countBadge').textContent = `${filtered.length} من ${students.length}`;
}
...
async function pushAttendanceData() {
    ...
    if (!dt) return alert(translations[currentLang].select_date_first);
...
async function syncAttendanceToGitHub(commitMessage = 'Update attendance data') {
    ...
    try {
        ...
        if (msg) {
            ...
            alert(`${translations[currentLang].sync_failed}: ${e.message}`);
        }
    } finally {
        btn.disabled = false;
...
async function addNewSession() {
    ...
    if (existing) {
        alert(translations[currentLang].session_exists);
...
async function deleteCurrentSession() {
    ...
    if (confirm(translations[currentLang].confirm_delete_session + ` (${activeContextDate})`)) {
...
async function clearCurrentSessionData() {
    if (!activeContextDate) return;
    if (confirm(translations[currentLang].confirm_clear_session + ` (${activeContextDate})`)) {
...
function transformLessonUI(lessonId, id) {
    ...
    if (infoSpan) {
        infoSpan.textContent = translations[currentLang].video_available;
    ...
    actionsDiv.innerHTML = `
        <a href="${watchLink}" target="_blank" rel="noopener noreferrer" class="action-btn-p watch">
            ...
            <span>${translations[currentLang].watch}</span>
        </a>
...
async function syncCurriculumToGitHub(commitMessage = 'Update curriculum data') {
    ...
    try {
        ...
        alert(`${translations[currentLang].curriculum_sync_error}: ${e.message}`);
    }
}

function promptVideoLink(lessonId) {
    if (!isLevelAdmin()) return alert(translations[currentLang].admin_required);

    const link = prompt(translations[currentLang].prompt_drive_link);
    if (!link) return;

    const idMatch = link.match(/\/d\/([-\w]{25,})/) || link.match(/id=([-\w]{25,})/);
    if (!idMatch) return alert(translations[currentLang].invalid_drive_link);
...
function promptQuizLink(lessonId) {
    ...
    const link = prompt(translations[currentLang].prompt_quiz_link);
    if (!link) return;
...
function transformQuizUI(lessonId, url) {
    ...
    const infoSpan = card.querySelector('.lesson-file-count');
    if (infoSpan) {
        infoSpan.textContent = translations[currentLang].quiz_ready;
        infoSpan.classList.add('video-available');
    }
...
function renderCalendar() {
    ...
    monthYearEl.textContent = new Intl.DateTimeFormat(currentLang === 'ar' ? 'ar-EG' : 'en-US', { month: 'long', year: 'numeric' }).format(currentViewDate);
        ...
        if (localRes && localRes.ok) {
            ...
            if (contentType && contentType.includes('application/json')) {
                try {
                    ...
                    console.warn("Failed to parse local levels JSON:", e);
                }
            } else {
                console.warn("Local levels data returned non-JSON content. Usually happens if path is missing and server fallbacks to HTML.");
            }
        } 
...
function openAddStudentModal() {
    document.getElementById('modalTitle').textContent = translations[currentLang].add_student_modal;
...
function openEditStudentModal(index) {
    ...
    if (!s) return;
    document.getElementById('modalTitle').textContent = translations[currentLang].edit_student_modal;
...
async function deleteStudent(index) {
    if (!confirm(translations[currentLang].confirm_delete_student)) return;
...
let currentLang = localStorage.getItem('eduTrack_lang') || 'ar';
...
function applyLanguage() {
    ...
    // Refresh dynamic lists that use translations
...
const translations = {
    ar: {
        ...
        filter_good: "مستوى جيد",
        filter_attention: "يحتاج اهتماماً",
        th_rank: "#",
        th_name: "الاسم",
        th_score: "الدرجة",
        th_class: "الفصل",
        th_guardian: "ولي الأمر",
        th_contact: "التواصل",
        th_actions: "إجراءات",
        no_students: "لم يتم العثور على طلاب",
        adjust_search: "جرّب تعديل البحث أو معايير التصفية.",
        audio_content: "المحتوى الصوتي",
        audio_desc: "استمع إلى المواد الصوتية المتاحة أو قم بتحميلها.",
        course_curriculum: "منهج الدورة",
        course_desc: "الأكاديمية - المستوى الثالث — تصفح الوحدات والدروس.",
        unit: "الوحدة",
        lessons_rev: "دروس + مراجعة",
        lesson: "الدرس",
        video_available: "المحتوى متاح",
        no_files: "لا توجد ملفات بعد",
        watch: "مشاهدة",
        download: "تحميل",
        revision: "مراجعة",
        attendance_title: "حضور الحصص",
        attendance_desc: "تتبع حضور الطلاب ومراجعة الجلسات السابقة",
        results: "النتائج",
        save_attendance: "حفظ الحضور",
        history_title: "الجلسات الأخيرة",
        present: "حاضر",
        absent: "غائب",
        ...
        session_cleared: "تم مسح البيانات",
        confirm_delete_student: "هل أنت متأكد من حذف هذا الطالب؟",
        confirm_delete_session: "هل أنت متأكد من حذف هذه الجلسة؟",
        confirm_clear_session: "هل أنت متأكد من مسح بيانات الجلسة؟",
        select_date_first: "يرجى اختيار تاريخ أولاً.",
        sync_failed: "فشل المزامنة",
        session_exists: "توجد جلسة لهذا التاريخ بالفعل. يتم تحميل بياناتها...",
        curriculum_sync_error: "خطأ في مزامنة المنهج",
        admin_required: "يجب أن تكون مسؤولاً لهذا المستوى للقيام بهذا الإجراء.",
        prompt_drive_link: "أدخل رابط فيديو Google Drive:",
        invalid_drive_link: "تنسيق رابط Google Drive غير صالح.",
        prompt_quiz_link: "أدخل رابط الاختبار (Google Form, إلخ):",
        add_student_modal: "إضافة طالب جديد",
        edit_student_modal: "تعديل بيانات الطالب",
        student_name: "اسم الطالب",
        score: "الدرجة",
        class: "الفصل",
        guardian_name: "اسم ولي الأمر",
        whatsapp_placeholder: "مثال: 201234567890",
        cancel: "إلغاء",
        save: "حفظ الطالب",
    ...
    en: {
        ...
        course_desc: "English Level 3 — Browse units and lessons",
        unit: "Unit",
        lessons_rev: "Lessons + Revision",
        lesson: "Lesson",
        ...
        download: "Download",
        revision: "Revision",
        attendance_title: "Class Attendance",
        attendance_desc: "Track student presence and review sessions",
        results: "Results",
        ...
        confirm_clear_session: "Are you sure you want to clear session data?",
        ...
        curriculum_sync_error: "Curriculum Sync Error",
        admin_required: "Admin privileges required for this level.",
        prompt_drive_link: "Enter Google Drive video link:",
        invalid_drive_link: "Invalid Google Drive link format.",
        prompt_quiz_link: "Enter Quiz link (Google Form, etc.):",
        ...
        whatsapp_placeholder: "Example: 201234567890",
        cancel: "Cancel",
        save: "Save Student",
        copy_success: "Copied!",
        copy: "Copy",
...
function applyLanguage() {
    ...
    // Toggle label visibility in switcher
    const arLabel = document.querySelector('.lang-label-ar');
    const enLabel = document.querySelector('.lang-label-en');
    if (arLabel && enLabel) {
        if (currentLang === 'ar') {
            arLabel.classList.remove('hidden');
            enLabel.classList.add('hidden');
        } else {
            arLabel.classList.add('hidden');
...
// Intercept init to apply language
